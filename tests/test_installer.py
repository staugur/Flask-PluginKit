...
class PITest(unittest.TestCase):
    ...
    def test_addPlugin(self):
        ...
        #local
        res = self.pi.addPlugin(method='local', filepath='/data/Desktop/1.tar', remove=True)
        self.assertEqual(res["code"], 1)
        res = self.pi.addPlugin(method='local', filepath='/data/Desktop/1.tbz', remove=True)
        self.assertEqual(res["code"], 1)
        res = self.pi.addPlugin(method='local', filepath='/data/Desktop/1.tgz', remove=True)
        self.assertEqual(res["code"], 0)
        res = self.pi.addPlugin(method='local', filepath='/data/Desktop/1.zip', remove=True)
        self.assertEqual(res["code"], 0)
        #remote
