...
class PluginInstaller(object):
    ...
    def remote_download(self, url):
        """下载远程文件，filename设置依照优先级有四种方法，每一种获取到合格的文件名时停止设置，最终无法获取合格有效的文件名时触发异常
            ...
            try:
                ...
                if filename and self.__isValidFilename(filename):
                    ...
                    with NamedTemporaryFile(mode='w+b', prefix='fpk-', suffix=suffix, delete=False) as fp:
                        fp.write(f.read())
                        filename = fp.name
                    print filename,os.path.isfile(filename),"is zip",zipfile.is_zipfile(filename),"is gz",tarfile.is_tarfile(filename)
                    try:
